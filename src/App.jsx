...
export default function App() {
    ...
    if (loading) return (
        <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-blue-400 font-black uppercase italic tracking-widest gap-4">
            <RefreshCcw className="animate-spin" size={32} /> Actualizando Radar Normativo...
        </div>
    );
                ...
                <div className="w-full max-w-md bg-slate-900 border border-slate-800 p-10 rounded-[2rem] shadow-2xl">
                    <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                            ...
                            <p className="text-[9px] text-slate-500 uppercase font-bold tracking-widest">Información Normativa Proactiva</p>
                        </div>
                    </div>
                    <form onSubmit={handleAuth} className="space-y-4">
                        <div className="space-y-1">
                            <label className="text-[9px] font-black text-slate-500 uppercase ml-1 block">Email Empresa</label>
                            <input type="email" placeholder="administracion@empresa.com" className="w-full p-4 bg-black border border-slate-800 rounded-xl text-white outline-none focus:border-blue-500 transition-all" value={email} onChange={e => setEmail(e.target.value)} required />
                        </div>
                        <div className="space-y-1">
                            <label className="text-[9px] font-black text-slate-500 uppercase ml-1 block">Contraseña</label>
                    ...
                    <button onClick={() => setAuthMode(authMode === 'login' ? 'register' : 'login')} className="w-full mt-6 text-slate-500 text-[10px] font-bold uppercase tracking-widest hover:text-white transition-all text-center">
                        {authMode === 'login' ? '¿Sin acceso? Solicítelo aquí' : '¿Ya tiene cuenta? Identifíquese'}
                    </button>
                </div>
            </div>
        );
                ...
                <header className="mb-8 bg-slate-900/80 p-6 rounded-3xl border border-slate-800 flex flex-wrap justify-between items-center gap-6 shadow-xl">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="text-right hidden sm:block border-r border-slate-800 pr-6">
                            <p className="text-[9px] text-slate-500 uppercase font-black tracking-widest mb-1">Empresa</p>
                            ...
                            {ayudasConMatch.map(ayu => (
                                <button key={ayu.id} onClick={() => setSelectedId(ayu.id)} className={`w-full p-5 rounded-[1.8rem] border text-left transition-all duration-300 ${selectedId === ayu.id ? 'bg-blue-600 border-blue-400 shadow-2xl scale-[1.02]' : 'bg-slate-900 border-slate-800 hover:border-slate-600 hover:bg-slate-800/50'}`}>
                                    ...
                                    <div className="mt-4 flex justify-between items-center border-t border-white/5 pt-3">
                                        <div>
                                            <p className="text-[7px] text-slate-400 uppercase font-black mb-0.5">Score de Afinida</p>
                    ...
                    <main className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-6 md:p-10 shadow-2xl relative min-h-[600px]">
                        {selectedHelp ? (
                            <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                                <div className="flex justify-between items-start mb-10 flex-wrap gap-6 border-b border-slate-800 pb-8">
                                    ...
                                    <div className="bg-slate-950 p-6 rounded-[2rem] border border-slate-800 text-center min-w-[140px] shadow-inner">
                                        <p className="text-[9px] text-slate-500 uppercase font-black mb-1 tracking-[0.2em]">Estimación Técnica</p>
                                ...
                                <div className="space-y-8 mb-12">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        ...
                                        <div className="bg-slate-950/50 p-8 rounded-[2rem] border border-slate-800/50">
                                            <p className="text-white font-black text-[10px] uppercase mb-6 flex items-center gap-2 tracking-widest"><FileText size={18} className="text-emerald-500" /> Documentación Orientativa</p>
                                            <ul className="space-y-4">
                                ...
                                {/* BLOQUE DE ACCIÓN REORIENTADO AL ASESOR */}
                                <div className="flex flex-col gap-4 pt-4">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-4 items-center">
                                        ...
                                        >
                                            <ExternalLink size={14} /> Referencia Técnica (BOE/SEPE)
                                        </a>
                                        ...
                                        <div className="flex-1 flex items-center gap-2 px-2 bg-amber-500/5 py-3 rounded-xl border border-amber-500/10">
                                            ...
                                            <p className="text-[9px] text-slate-500 font-medium leading-tight italic">
                                                Advertencia: El análisis normativo es informativo. La gestión final requiere la validación técnica de su despacho profesional.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="flex flex-col items-center justify-center h-full text-center space-y-6 opacity-40">
                                <div className="p-8 bg-slate-800/30 rounded-full border border-dashed border-slate-700"><Sparkles size={60} className="text-slate-600 animate-pulse" /></div>
                                <div className="space-y-2">
                                    <p className="text-[11px] uppercase font-black italic tracking-[0.3em] text-slate-500">Radar en espera</p>
                                    <p className="text-xs text-slate-600 font-medium">Seleccione un boletín para iniciar el resumen ejecutivo</p>
                                </div>
                            </div>
                        )}
                    </main>
